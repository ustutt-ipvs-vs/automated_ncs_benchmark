...
int main(int argc, char* argv[]){
    ...
    TokenBucket tokenBucket(2'000'000, 0.125, 0);
    ...
    while(true){
        if(tokenBucket.getBucketLevel() < payloadSize){
        ...
        if(packetCount % 500 == 0){
            std::cout << "Sent " << packetCount << " packets. Bucket Level: " << tokenBucket.getBucketLevel() << std::endl;
        }
    }
}