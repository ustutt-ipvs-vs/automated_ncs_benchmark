...
## In this version of the protocol, all replicas send suggestions for the proposal to the coordinator
...
# Replace `IV` and recalculate `loss` of estimate `e` if necessary
function Estimate(e::Estimate, IV::Vector{Float64}, r::NCSReplica)
    if all( isnan(a) == isnan(b) for (a,b) in zip(e.IV,IV) )
        return e
    else
        return Estimate(e.id,e.v,e.vp,e.kp,e.k,e.S,IV,predict_loss(e.S.covariance,IV,r.config.sys))
    end
end
Estimate(::Missing, ::Vector{Float64}, ::NCSReplica) = missing

# Process for computing and issuing setpoints
@resumable function replica_compute_proc_allprop(env::Environment, r::NCSReplica, actuator_queue::Store{ActuatorMessage}, log_available::BitMatrix, OVplant=nothing)
    @assert env === r.env
    m = nothing
    next_event = nothing
...
isdebuglogged() && (@printtime env; printreplica(r); println(stderr," update with OV(",r.state.k-1,")=",isnothing(OVact) ? "[default]" : OVact," IV=",r.IV))
    ...
    # @printtime env; printreplica(r); println(stderr," STARTING CONSENSUS for period k=",r.state.k)
        ...
        #########################################################################
        fresh_period = true
        decide_timeout = timeout(env, 4*r.config.δn, priority=-1) # Suggest(Estimate) + Propose + Acknoledge + Decide
        #########################################################################
        ...
        while true #in_period || !isprocessed(receive_timeout) # LOOP OVER VIEWS
            ...
            #########################################################################
            if fresh_period && r.state.cid > 0
                # in the first round of each period, everyone sends an estimate
                send(r.env, r.group[r.state.cid], Estimate(r), r.id)
            end
            #########################################################################
    ...
    # @printtime env; printreplica(r); println(stderr," PROPOSE    k=",r.state.k," v=",r.state.v," ke=",r.state.ke)
                    #########################################################################
                    if fresh_period
                        # in the first round of each period, the coordinator collects "dummy" estimates to improve the IV
                        receive_timeout = timeout(env, r.config.δn, priority=-1)
                        while true
                            ...
                            if isprocessed(get_msg) # received a message
                                m = value(get_msg)
                                if iscurrent(r, m) && m isa Estimate && m.k == r.state.k - 1
                                    r.estimates[m.id] = m
                                end
                            else
                                SimJulia.cancel(r.replication_queue, get_msg)
                            end
                            if isprocessed(receive_timeout) || iszero(count(ismissing, r.estimates))
                                break
                            end
                        end
                        copyto!(r.IV, mergeIV(r.estimates))
                        r.estimates .= Estimate.(r.estimates, (r.IV,), (r,))
                        est = maximum(skipmissing(r.estimates))
                        accept!(r, est)
