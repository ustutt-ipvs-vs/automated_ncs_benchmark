...
export SimConfig, SimResult, typedict,
    ...
    Replica, NCSReplica, SensorMessage, ActuatorMessage
...
include("src/replica-proc-allprop.jl")
include("src/replica-proc-share-iv.jl")
...
## TODO: make `share_OV` and `hold` fields of `SimConfig`
...
function runsim(config::SimConfig, until::Real, seed=nothing, failseed=seed; share_OV::Bool=true, hold::Bool=true, Tmin::Real=10, Tmax::Real=600, quiet::Bool=false, logfields::NTuple{N,Symbol}=(:g,:p), variant::Symbol=:default)::SimResult where N
    ...
    log_messages = Ref(0)
    replicas = Vector{NCSReplica}(sim, config.g, config, log_messages, simsteps, failseed)
    ...
    for r in replicas
        ...
        if variant == :default
            @process replica_compute_proc(sim, r, actuator_queue, log_available, OVreplica)
        elseif variant == :allprop
            @process replica_compute_proc_allprop(sim, r, actuator_queue, log_available, OVreplica)
        elseif variant == :shareIV
            @process replica_compute_proc_shareIV(sim, r, actuator_queue, log_available, OVreplica)
        end
    end
